    ...
    with open(fasta_file, "rU") as input_handle:
        ...
        for hit in hits:
            ...
            try:
                ...
                print(
                    f"ERROR: No matches found in FASTA file for hit {hit_name}"
                )
                sys.exit(-1)
            if (hit_name != seq_dict[hit_name].id):
                print(
                    f"ERROR: Hit name and FASTA file mismatch between {hit_name} and {seq_dict[hit_name].id}"
                )
                sys.exit(-1)
            print(f">{hit_name}")
            print(f"{hit}")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="BLAST command to run. Must have outfmt 0 specified.")
    parser.add_argument(
        "-f",
        "--fasta-file",
        type=str,
        required=True,
        help=
        "File in FASTA format containing any sequences that BLAST may return as hits."
    )
    args = parser.parse_args()
    hits = get_blast_cmd_output(args.blast_command)
    if len(hits) == 0:
        print(
            f"ERROR: No hits returned by BLAST for command {args.blast_command}"
        )
        sys.exit(-1)
